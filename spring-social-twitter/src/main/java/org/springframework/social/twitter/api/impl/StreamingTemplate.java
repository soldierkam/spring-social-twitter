...
class StreamingTemplate extends AbstractTwitterOperations implements StreamingOperations {
    ...
    public void filter(final String trackKeywords, final List<StreamListener> listeners) {
        ...
        consumer = new StreamConsumer() {
            protected Stream getStream() throws StreamCreationException {
                return createFilterStream(trackKeywords, listeners);
            }
        };
    ...
    public void sample(final List<StreamListener> listeners) {
        ...
        consumer = new StreamConsumer() {
            protected Stream getStream() throws StreamCreationException {
                return createSampleStream(listeners);
            }
        };
        startNewConsumer(consumer);
    }

    private Stream createFilterStream(String trackKeywords, List<StreamListener> listeners) throws StreamCreationException {
        try {
    ...
    private Stream createSampleStream(List<StreamListener> listeners) throws StreamCreationException {
        try {
            MultiValueMap<String, String> requestBody = new LinkedMultiValueMap<String, String>();
            ClientHttpResponse response = executeRequest(HttpMethod.POST, STREAM_SAMPLE_URL, requestBody);
            if (response.getStatusCode().value() > 200) {
                throw new StreamCreationException("Unable to create stream", response.getStatusCode());
            }
            return new StreamImpl(response.getBody(), listeners);
    ...
    private static final String STREAM_FILTER_URL = "https://stream.twitter.com/1/statuses/filter.json";
    private static final String STREAM_SAMPLE_URL = "https://stream.twitter.com/1/statuses/sample.json";
